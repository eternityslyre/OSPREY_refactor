	...
	private void readBranchDecomposition(String fName, Molecule m){
		...
		for (int i=0; i<tn.length; i++){ //read in the information for each node
			...
			if (tnIsLeaf) { //leaf node
				int tnV1 = m.getResByPDBResNumber(getToken(str,4)).indexInMolecule; //transform to molecule-relative numbering
				int tnV2 = m.getResByPDBResNumber(getToken(str,5)).indexInMolecule;
